...
def _post_entities(base_url: str, headers: Dict[str, str], entity_type: str, options: Dict[str, Any]) -> Tuple[List[str], int]:
    """Call entityappservice/get with options and return (ids, http_status)."""
    body = {
        "params": {
    ...
    def yield_new_from(iterator: Iterable[List[str]], label: str) -> int:
        ...
        for ids in iterator:
            ...
            if new_ids:
                ...
                hb(f"{label}: duplicate-only page; switching soon if this persists…")
        return new_gained
...
if btn_reeval:
    ...
    expected_global = sum(int((st.session_state.type_counts.get(et) or {}).get("totalRecords") or 0)
                          for et in selected) if counts_available else None

    overall = st.progress(0.0)
    status_area = st.empty()
    hb_area = st.empty()

    status_area.info("Starting re-evaluation… preparing preflight checks so you see progress immediately.")

    # Preflight pings (fast)
    ...
    for et in selected:
        ...
        if s >= 400:
            preflight_fail = True
            st.error(f"Preflight failed for `{et}` (HTTP {s}). Check tenant/credentials and try again.")
            break
        else:
    ...
    # Heartbeat using mutable box (no nonlocal)
    last_hb = [time.time()]
    def heartbeat(msg: str):
        now = time.time()
        if now - last_hb[0] >= 2.0:
            hb_area.info(f"⏳ {msg}")
            last_hb[0] = now
    ...
    if counts_available:
        ...
        if missing:
            st.warning("⚠️ Re-evaluation finished with deficits (processed < expected):")
            for et, deficit, exp, got in missing:
