...
# ---------------------------------------------
# Paging strategies (no heartbeat lines)
# ---------------------------------------------
...
def fetch_ids_resilient(base, headers, etype, expected_total):
    """
    Yields chunks of *new* IDs for a type using:
      1) pageNumber@2000 → 2) offset@2000 → 3) sweeps at 1000, 500.
    Ensures no duplicates and stops only when expected_total reached or pages exhausted.
    """
    seen=set()
    def only_new(seq):
        for ids in seq:
...
if btn_start:
    ...
    # (Change #1) — Remove preflight OK lines: we silently probe reachability but do not print anything.
    for et in selected:
    ...
    with ThreadPoolExecutor(max_workers=REEVAL_WORKERS) as pool:
        for et in selected:
            ...
            for new_ids in fetch_ids_resilient(BASE_URL,headers,et,expected_total):
                ...
                # consume futures one-by-one -> update the live line on **each** completion
                for fut in as_completed(futures):
                    ...
                    # increment exactly once per unique id (safety)
                    if res["id"] not in unique_seen_ids:
                        unique_seen_ids.add(res["id"])
                        processed_this_type += 1
                        done_global += 1

                        # update global progress bar & text on every record
                        if denom:
                ...
                # stop early if we reached the known count
